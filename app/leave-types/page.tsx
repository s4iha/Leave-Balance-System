...
export default function LeaveTypesPage() {
  ...
  const leaveTypesQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<LeaveTypeListResponse>(`/api/v1/leave-types?${paramsString}`, undefined, user?.id, user?.email),
  ...
  useEffect(() => {
    ...
    showErrorToast(leaveTypesQuery.error, 'Failed to load leave types');
  ...
  const upsertLeaveTypeMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.leaveTypes.all, refetchType: 'none' });
      toast({
  ...
  const total = leaveTypesQuery.data?.total ?? 0;
  ...
  const parseRequiredInt = (value: string, field: string): number | null => {
    if (!value.trim()) {
      showErrorToast(`${field} is required.`, 'Validation Error');
      return null;
    }
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed <= 0) {
      showErrorToast(`${field} must be a positive whole number.`, 'Validation Error');
      return null;
    }
    return parsed;
  };
  ...
  const parseOptionalNonNegativeInt = (value: string, field: string): number | null => {
    ...
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed < 0) {
      showErrorToast(`${field} must be a non-negative whole number.`, 'Validation Error');
      return null;
    }
    return parsed;
  };
  ...
  const handleSaveLeaveType = async () => {
    ...
    if (!name) {
      showErrorToast('Leave type name is required.', 'Validation Error');
      return;
    }
    ...
    if (formState.carryoverAllowed && carryoverMaxDays !== null && carryoverMaxDays > maxDaysPerYear) {
      showErrorToast('Carryover max days cannot exceed max days per year.', 'Validation Error');
      return;
    }
              ...
              {leaveTypesQuery.isLoading ? (
                ...
                <>
                  <Card className="border-border">
                    ...
                    <CardContent>
                      <div className="text-4xl font-bold text-foreground">{leaveTypes.length}</div>
                    </CardContent>
                  </Card>
                      ...
                      {leaveTypesQuery.isLoading && (
                        <TableRow>
                          <TableCell colSpan={6} className="p-0 border-0">
                            <SkeletonTable rows={5} columns={6} className="p-4" />
                          </TableCell>
                        </TableRow>
                      )}
