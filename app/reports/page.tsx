...
export default function ReportsPage() {
  ...
  const canAccess = Boolean(user && user.permissions?.some(p => p.action === 'APPROVE' && p.resource === 'LEAVE_REQUEST'));
  useEffect(() => {
    if (!selectedYear) {
  ...
  const departmentsQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<DepartmentListResponse>('/api/v1/departments?skip=0&take=100', undefined, user?.id, user?.email),
    enabled: canAccess,
  ...
  const balanceReportQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<ReportResponse<BalanceReportItem[]>>(`/api/v1/reports?${balanceParams}`, undefined, user?.id, user?.email),
    enabled: canAccess,
    staleTime: 60 * 1000,
  });
  ...
  const accrualReportQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<ReportResponse<AccrualReportData>>(`/api/v1/reports?${accrualParams}`, undefined, user?.id, user?.email),
  ...
  const leaveTypeReportQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<ReportResponse<LeaveTypeReportData>>(`/api/v1/reports?${leaveTypeParams}`, undefined, user?.id, user?.email),
  ...
  useEffect(() => {
    ...
    showErrorToast(balanceReportQuery.error, 'Failed to load balance report data');
  ...
  useEffect(() => {
    ...
    showErrorToast(accrualReportQuery.error, 'Failed to load accrual report data');
  ...
  useEffect(() => {
    ...
    showErrorToast(leaveTypeReportQuery.error, 'Failed to load leave type report data');
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <div className="min-w-[120px]">
                  ...
                  >
                    <SelectTrigger className="bg-muted border-input h-9 text-xs">
                ...
                <>
                  <Card className="border-border">
                    ...
                    <CardContent className="pt-4">
                      <div className="text-4xl font-bold text-foreground">{totalBalance.toFixed(1)}</div>
                  ...
                  <Card className="border-border">
                    ...
                    <CardContent className="pt-4">
                      <div className="text-4xl font-bold text-foreground">{averageBalance.toFixed(1)}</div>
                  ...
                  <Card className="border-border">
                    ...
                    <CardContent className="pt-4">
                      <div className="text-4xl font-bold text-foreground">{usedThisYear.toFixed(1)}</div>
                  ...
                  <Card className="border-border">
                    ...
                    <CardContent className="pt-4">
                      <div className="text-4xl font-bold text-foreground">{approvedRequests ?? '-'}</div>
            ...
            <div className="mb-4 border-b border-border">
              <div className="flex gap-4">
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeAnalyticsTab === 'accrual'
                ...
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeAnalyticsTab === 'leave-type'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                      ...
                      {balanceReportQuery.isLoading && (
                        <TableRow>
                          <TableCell colSpan={8} className="p-0 border-0">
                            <SkeletonTable rows={5} columns={8} className="p-4" />
                          </TableCell>
                        </TableRow>
                      )}
