...
import { ClassificationsTab } from '@/components/settings/classifications-tab';
import { BalancePoliciesTab } from '@/components/settings/balance-policies-tab';
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'adjustments' | 'user-access' | 'holidays' | 'roles' | 'classifications' | 'balance-policies'>('adjustments');
  ...
  const canAccess = Boolean(user && user.permissions?.some(p => p.action === 'MANAGE' && p.resource === 'USER'));
  ...
  const adjustmentsQuery = useQuery({
    ...
    queryFn: () => apiRequestRaw<AdjustmentListResponse>(`/api/v1/adjustments?${adjustmentsParams}`, undefined, user?.id, user?.email),
    enabled: canAccess && activeTab === 'adjustments',
  });

  const employeesQuery = useQuery({
    queryKey: queryKeys.employees.list('skip=0&take=100&active=true'),
    queryFn: () => apiRequestRaw<EmployeeListResponse>('/api/v1/employees?skip=0&take=100&active=true', undefined, user?.id, user?.email),
    enabled: canAccess,
  });

  const leaveTypesQuery = useQuery({
    queryKey: queryKeys.leaveTypes.list('skip=0&take=100'),
    queryFn: () => apiRequestRaw<LeaveTypeListResponse>('/api/v1/leave-types?skip=0&take=100', undefined, user?.id, user?.email),
    enabled: canAccess,
  });
  ...
  useEffect(() => {
    ...
    showErrorToast(adjustmentsQuery.error, 'Failed to load adjustments');
  ...
  const confirmDeleteAdjustment = async () => {
    ...
    try {
      ...
      showErrorToast(error, 'Failed to delete adjustment');
    }
  };

  const handleSaveAdjustment = async () => {
    if (!formData.employeeId || !formData.leaveTypeId || !formData.adjustmentType || !formData.reason.trim()) {
      showErrorToast('Employee, leave type, adjustment type, and reason are required.', 'Validation Error');
      return;
    }

    const adjustmentDays = parseFloat(formData.adjustmentDays);
    if (Number.isNaN(adjustmentDays) || adjustmentDays === 0) {
      showErrorToast('Adjustment days must be a non-zero number.', 'Validation Error');
      return;
    }
    ...
    try {
      ...
      showErrorToast(error, 'Failed to save adjustment');
    }
  };
        ...
        <MainContent>
          <div className={cn("pt-4 pr-4 pb-4 pl-4 md:pt-8 md:pr-8 md:pb-8", isCollapsed ? "md:px-8" : "md:px-8", "max-w-8xl mx-auto")}>
            <div className="mb-8">
              ...
              <p className="text-muted-foreground mt-2">Manage leave adjustments and view system settings</p>
            </div>
            ...
            <div className="mb-6 border-b border-border">
              <div className="flex gap-4">
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'adjustments'
                ...
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'user-access'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'roles'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'holidays'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'classifications'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                ...
                <button
                  onClick={() => setActiveTab('balance-policies')}
                  className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === 'balance-policies'
                    ? 'border-primary text-primary'
                    : 'border-transparent text-muted-foreground hover:text-foreground'
                    }`}
                >
                          ...
                          {adjustmentsQuery.isLoading && (
                            <TableRow>
                              <TableCell colSpan={7} className="p-0 border-0">
                                <SkeletonTable rows={5} columns={7} className="p-4" />
                              </TableCell>
                            </TableRow>
                          )}
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="mt-2 bg-muted border-input min-h-20"
              />
            </div>
          </div>
          <DialogFooter>
